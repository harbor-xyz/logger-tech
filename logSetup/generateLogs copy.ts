...
const sendToNewRelic = (logEntry: LogEntry): void => {
  newrelic.startWebTransaction(logEntry.fullendpoint, function transactionHandler() {
    const transaction = newrelic.getTransaction();
    transaction.http.statusCode = parseInt(logEntry.status);
    transaction.http.method = logEntry.method;
    newrelic.addCustomAttribute('request_id', logEntry.request_id);
    newrelic.addCustomAttribute('level', logEntry.level);
    newrelic.addCustomAttribute('hostname', logEntry.hostname);
    newrelic.addCustomAttribute('job', logEntry.job);
    transaction.end();
  });
};
